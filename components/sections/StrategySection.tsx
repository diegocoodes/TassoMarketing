...
export function StrategySection() {
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const timeline = gsap.timeline({ defaults: { ease: "power3.out" }, scrollTrigger: { trigger: section, start: "top 72%", once: true } });

      timeline
        .fromTo("[data-strategy-glow]", { opacity: 0, scale: 0.82 }, { opacity: 1, scale: 1, duration: 0.7 })
        .fromTo("[data-strategy-orbit-path]", { strokeDashoffset: 1000 }, { strokeDashoffset: 0, duration: 0.9 }, "-=0.35")
        .fromTo("[data-strategy-person]", { opacity: 0, y: 28, scale: 1.04, clipPath: "inset(0 0 100% 0)" }, { opacity: 1, y: 0, scale: 1, clipPath: "inset(0 0 0% 0)", duration: 0.9 }, "-=0.5")
        .fromTo("[data-strategy-shine]", { xPercent: -170, opacity: 0 }, { xPercent: 260, opacity: 0.65, duration: 0.85 }, "-=0.18")
        .fromTo("[data-strategy-tag]", { opacity: 0, y: 18, scale: 0.94 }, { opacity: 1, y: 0, scale: 1, duration: 0.55, stagger: 0.09 }, "-=0.45")
        .fromTo("[data-strategy-copy]", { opacity: 0, y: 30 }, { opacity: 1, y: 0, duration: 0.65, stagger: 0.08 }, "-=0.38")
        .fromTo("[data-strategy-card]", { opacity: 0, y: 28 }, { opacity: 1, y: 0, duration: 0.58, stagger: 0.1 }, "-=0.28")
        .fromTo("[data-strategy-progress]", { scaleX: 0 }, { scaleX: 1, duration: 0.7, transformOrigin: "left" }, "-=0.35")
        .fromTo("[data-strategy-cta]", { opacity: 0, y: 22 }, { opacity: 1, y: 0, duration: 0.55 }, "-=0.18");

      gsap.to("[data-strategy-orbit]", { rotate: 360, duration: 36, repeat: -1, ease: "none", transformOrigin: "center" });

      mm.add("(min-width: 1024px)", () => {
        gsap.to("[data-strategy-visual-inner]", { y: -24, ease: "none", scrollTrigger: { trigger: section, start: "top bottom", end: "bottom top", scrub: 1.2 } });
      });
